...
const AddPeopleModal = ({
  ...
  return (
    <div
      className="absolut fixed top-0 z-20 w-full h-full flex justify-center items-center"
      style={{ backgroundColor: "rgba(0, 0, 0, 0.2)" }}
    >
      <div
        className={`w-[96%] md:w-[70%] lg:w-[45%] 2xl:w-[50%] md:h-[87%] flex flex-col justify-between bg-white rounded-xl p-3 lg:p-5 ${
      ...
      >
        <div className="">
          <div className="flex justify-start items-center text-[14px] lg:text-[16px] border-b pb-2 text-gray-600">
            <button className="px-5 py-1 rounded-md text-white border bg-[#649B8E]">
              Choose Classmate
            </button>
            <button
              onClick={handleAddByContactModal}
              className="px-5 py-1 rounded-md ml-5 border boredr-[#649B8E] text-[#649B8E]"
            >
              Add by Email/Phone
            </button>
          </div>
          <div className="">
            <p className="py-2">
              Graduation - QIS college of Engg & Tech, Tamilnadu
            </p>
            <input
              type="search"
            ...
            <div className="h-[190px] md:h-[205px] lg:h-[288px] 2xl:h-[320px] hideScroll overflow-y-scroll">
              {dataList.length !== 0 ? (
                dataList.map((data, i) => (
                  <div key={i} className="flex items-center mb-2 lg:mb-3">
                    ...
                    <div className="w-1/6 flex justify-end">
                      {selectAll ? (
                        <img src={selectedimg} className="h-6 w-6" />
                      ) : (
                ...
                <p className="h-full flex justify-center items-center bg-sky-50">
                  No {education} friends were found
                </p>
              )}
            </div>
          </div>
        ...
        <section className="flex w-full">
          ...
          <div className="w-5/6">
            ...
            <button
              onClick={handlePeopleModalClose}
