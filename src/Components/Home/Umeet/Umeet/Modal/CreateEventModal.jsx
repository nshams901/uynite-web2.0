...
const CreateEventModal = ({
  ...
  handleShowTemplate,
  handleShowAddGroup,
  handleShowAddPoliticalGroup,
  whichType,
  politicalPartyFeedback,
  politicalPartyMeeting,
  handlePoliticalFeedbackQuestion,
  publicShopOpening,
  ...
  const postData = {
    ...
    eventdateAndTime: new Date(formState.eventdateAndTime).toLocaleString(
      "en-US",
      options
    ),
  ...
  return (
    <div className="lg:fullPage pb-16 lg:pb-3 bg-white border-gray-300">
      <div
        className={`${
          editMyEvent ? "lg:w-[65%]" : "w-full md:w-[96%]"
        } border bg-white md:px-2 lg:px-3`}
      >
        <section className="flex justify-between items-center">
          {editMyEvent ? (
            <div className="px-3 my-2.5 text-[17px] font-semibold">
              Edit Event
            </div>
          ) : (
            <div className="px-3 my-2.5 text-[17px] font-semibold">
          ...
          {selectedImage && (
            <AiOutlineEye
              ...
              className="mr-3 w-6 h-6 text-gray-700 cursor-pointer"
            />
          )}
        </section>
        ...
        <div className="px-7">
          {editMyEvent ? (
            ...
            <>
              <p className="pt-4 pb-2 text-[#649B8E]">
                {selectedSpecificEvent}
              </p>
              <hr />
            </>
          ...
          <span
            ...
            className="flex cursor-pointer justify-center py-2 text-[#649B8E]"
          >
            Select Template
          </span>
          <input
          ...
          <div
            className={`${
              politicalPartyFeedback || publicShopOpening ? "hidden" : ""
          ...
          {eventMode == "location" && (
            <div
              className={`${
                politicalPartyFeedback || politicalPartyMeeting ? "hidden" : ""
              } w-full my-3`}
            >
              <AutocompletePlace
              ...
              <input
                ...
                className={`${
                  politicalPartyFeedback || politicalPartyMeeting
                    ? "hidden"
                    : ""
                } border-b border-gray-300 h-10 my-2 w-full focus:outline-none`}
          ...
          {!isValid && (
            <div className="text-xs flex justify-center text-red-600">
          ...
          <div className="flex justify-between items-center my-2">
            <div className="flex items-center">
              <img src={guest} />
              <label
                onClick={handleGroupAndCreate}
                className={`${
                  umeetReducer.inviteEmailsUI ? "hidden" : ""
                } pl-5 cursor-pointer text-[#649B8E]`}
              >
                Add Guests
              </label>
              <label
              ...
              >
                {umeetReducer?.inviteEmailsUI?.length} Guests Added
              </label>
            </div>
            <span
              onClick={handleEditAdd}
              className={`${
                umeetReducer.inviteEmailsUI ? "" : "hidden"
              } cursor-pointer text-[#649B8E] border border-[#649B8E] px-2 py-0.5 rounded-md`}
            >
              Edit List
            </span>
          </div>
          ...
          >
            <select className="h-10 outline-none w-full border-b bg-white text-gray-600">
              ...
              <option className={`${publicShopOpening ? "hidden" : ""}`}>
                Guest List - Display to all
              </option>
              <option className={`${publicShopOpening ? "hidden" : ""}`}>
                Guest List - Display only to host
              </option>
            </select>
          </div>
          ...
          <div
            className={`${
              eventMode == "online" || politicalPartyFeedback ? "hidden" : ""
            } flex my-4 justify-between`}
          >
            <span className="text-gray-700">Food Availability</span>
            <div className="">
              <ToggleButton handleFoodCreate={(value) => setIsveg(value)} />
            </div>
          </div>
          {isVeg && (
            <div
          ...
          <textarea
            ...
            className={`${
              characterCount > 200 ? "outline-red-100" : ""
            } w-full text-gray-700 outline-none my-2 rounded-xl relative border p-2`}
          />
          <label
            className={`${
              characterCount > 200 ? "error" : ""
          ...
          <div className={`${politicalPartyFeedback ? "" : "hidden"} `}>
            <p
              onClick={handlePoliticalFeedbackQuestion}
              className="py-2 font-bold text-[18px] cursor-pointer text-[#519d8b]"
            >
          ...
          <div className="flex flex-col my-1">
            {editMyEvent ? (
              <button
                onClick={() => dispatch(updateEvent(postData))}
                className="py-2.5 my-2 text-[17px] rounded-lg text-white font-semibold bg-[#649B8E] "
              >
                Update
              </button>
            ) : (
            ...
            <button className="py-2 text-[17px] rounded-lg font-semibold border border-[#649B8E]">
              Cancel
            </button>
          </div>
        </div>
