...
const EventDeleteModal = ({ onClose }) => {
  ...
  return (
    <section
      className="fixed z-20 justify-center items-center top-0 left-0 h-full w-full flex"
    ...
    >
      <div className="w-[86%] md:w-[40%] lg:w-[42%] xl:w-[30%] h-[50%] flex flex-col justify-between p-4 bg-white md:ml-[9%] mt-[4%] rounded-2xl">
        ...
        <textarea
          ...
          placeholder="Tell the reason for that..."
        />
        <button
          onClick={() => dispatch(cancelEvent(deleteData))}
          className="px-5 py-2 rounded-lg font-semibold my-1 text-white border bg-[#649B8E]"
        >
          Delete
        </button>
      </div>
